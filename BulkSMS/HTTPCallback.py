...
def fetch_reply(auth_field=None, auth_value=None):
    ...
    for key in list(silly_form.keys()):
    ...
    if auth_field is not None and auth_value is not None:
        ...
        if auth is None:
            raise AuthenticationException('Missing auth field: %r' % ( auth_field ))
    ...
    try:
        ...
        raise FormatException('HTTP relay message missing field %r' % ( e.args[0] ))

    except ValueError as e:
        raise FormatException('HTTP relay msg_id or referring_msg_id not an integer.')
